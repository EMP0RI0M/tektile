...
export function WorkspaceFrame({ children }: { children: React.ReactNode }) {
  ...
  const isSystemRoute = ["dashboard", "project", "signin", "signup"].includes(firstPath || "");
  const routeRepoId = isSystemRoute ? null : (firstPath ?? null);
  const routeConversationId = isSystemRoute ? null : (pathParts[1] ?? null);
