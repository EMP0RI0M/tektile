...
export async function analyzeEditIntentAI(
  ...
  apiKey?: string
): Promise<EditIntent & { searchPlan?: SearchPlan }> {
  try {
    ...
    const targetFiles = foundPaths.filter(p => 
      filePaths.includes(p.startsWith('/') ? p.slice(1) : p) || filePaths.includes(p)
    ...
    // Use Gemini for structured output as it is more reliable for JSON mode on OpenRouter
    ...
    return {
      type: plan.editType as any,
      targetFiles: targetFiles.length > 0 ? targetFiles : [manifest.entryPoint], 
      confidence: 1.0,
...
/**
 * Analyze user prompts to determine edit intent and select relevant files (Heuristic)
 */
